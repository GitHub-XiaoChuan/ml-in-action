...
def inception_v3_arg_scope(weight_decay=0.00004,
    ...
    :param weight_decay: L2正则的参数weight_decay
    :param stddev: 标准差
    ...
    # 自动对slim.conv2d, slim.fully_connected进行赋值
    # 并将其中的参数weights_regularizer默认值设置为slim.l2_regularizer(weight_decay)
...
def inception_v3_base(inputs, scope=None):
    ...
    with tf.variable_scope(scope, 'InceptionV3', [inputs]):
        # 设置默认的参数，默认padding方式为Valid，步长为1；如果有修改的再单独赋值
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
            ...
            with tf.variable_scope('Mixed_6a'):
                with tf.variable_scope('Branch_0'):
                    # 步长为2，大小压缩，通道数增加 17*17*384
                    branch_0 = slim.conv2d(net, 384, [3, 3], stride=2, padding='VALID', scope='Conv2d_1a_1x1')
                with tf.variable_scope('Branch_1'):
                    # 最后一步的步长为2， 输出为 17*17*96
                    branch_1 = slim.conv2d(net, 64, [1, 1], scope='Conv2d_0a_1x1')
                    branch_1 = slim.conv2d(branch_1, 96, [3, 3], scope='Conv2d_0b_3x3')
                    branch_1 = slim.conv2d(branch_1, 96, [3, 3], padding='VALID', stride=2, scope='Conv2d_1a_1x1')
                with tf.variable_scope('Branch_2'):
                    # 就是一个最大池化层，步长为2，输出为 17*17*256
            ...
            with tf.variable_scope('Mixed_6b'):
                ...
                with tf.variable_scope('Branch_1'):
                    ...
                    branch_2 = slim.conv2d(branch_1, 192, [7, 1], scope='Conv2d_0c_7x1')
                with tf.variable_scope('Branch_2'):
