...
class Block(collections.namedtuple('Block', ['scope', 'unit_fn', 'args'])):
    'A named tuple describeing a Resnet block.'
...
def stack_blocks_dense(net, blocks, outputs_collections=None):
    ...
    :param net:
    :param blocks: 参数列表
    :param outputs_collections: 搜集各个end_points的collections
    :return:
    """
    for block in blocks:
...
def bottleneck(inputs, depth, depth_bottleneck, stride, outputs_collections=None, scope=None):
    ...
    :param inputs:              输入
    :param depth:               blocks arg
    :param depth_bottleneck:    blocks arg
    :param stride:              blocks arg
    :param outputs_collections: 搜集Unit名称
    :param scope:               名称
    :return:
    """
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        # 在每层之前都是用BN, 进行preactivation
        preact = slim.batch_norm(inputs, activation_fn=tf.nn.relu, scope='preact')

        # 定义shortcut，看看是否需要调整通道数
        if depth == depth_in:
